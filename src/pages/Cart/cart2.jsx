...
import { getData, getDataDrink, getProductById, getProductdrinkById, postCart } from "../../reduser/counter";
...
const Cart = () => {
  ...
  const { id } = useParams();
  const ById = useSelector((state) => state.product.drinkById);
  const product = useSelector((state) => state.product.dataDrink);
  const dispatch = useDispatch();

console.log(ById);

  useEffect(() => {
    dispatch(getProductdrinkById(id));
  }, [id]);
  ...
  return (
    <div className="py-[10%] mt-[100px] w-[90%] m-auto flex flex-col gap-[40px] ">
      <div className="flex gap-[70px] items-start sm:flex-col sm:">
        <div className="flex items-start gap-[30px] sm:gap-[12px] sm:flex-col-reverse">
          <img
            src={`${imgUrl}${ById.drink?.pathPhoto}`}
        ...
        <div className="w-[320px] flex flex-col gap-[33px] ">
          <div className="flex flex-col gap-[21px] sm:gap-[12px]">
            ...
            <h1 className="text-[18px] font-[400] text-gray-400 font-mono sm:text-[20px] ">
              {ById.drink?.description}
            </h1>
            <h1 className="text-[18px] font-[400] text-gray-400 font-mono sm:text-[20px] ">
              {ById.drinkIngredients?.map((e) => {
                return e.name + ",";
              })}
            </h1>
            ...
            <div className="flex items-center gap-[24px]">
              ...
              <h1 className="text-[18px] font-[400] text-green-500 font-mono sm:text-[20px] ">
                {ById.drink?.cookingTimeInMinutes * cnt}min
              </h1>
            </div>
            <hr />
          </div>
          <h1 className="text-[25px] font-[600] sm:text-[24px] sm:font-[700]">
            {ById.drink?.price * cnt}$
          </h1>
